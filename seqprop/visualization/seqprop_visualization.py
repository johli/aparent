...
def plot_seqprop_logo(pwm, iso_pred, cut_pred, annotate_peaks=None, sequence_template=None, figsize=(12, 3), width_ratios=[1, 7], logo_height=1.0, usage_unit='log', plot_start=0, plot_end=164, save_figs=False, fig_name=None, fig_dpi=300) :
	...
	if annotate_peaks is not None :
		...
		annot_text = '(CSE+' + str(objective_pos + plot_start - 76 + 0) + ') ' + str(int(round(cut_pred[objective_pos] * 100, 0))) + '% Cleavage'
...
class SeqPropMonitor(Callback):
    def __init__(self, predictor, plot_every_epoch=False, track_every_step=False, isoform_start=80, isoform_end=110, pwm_start=0, pwm_end=100, sequence_template='', plot_pwm_indices=[]) :
        ...
        _, optimized_pwm, _, _, cut_pred, _, _ = self.predictor.predict(x=None, steps=1)
    ...
    def on_epoch_end(self, epoch, logs={}) :
        ...
        if self.plot_every_epoch :
            ...
            for pwm_index in self.plot_pwm_indices :
                ...
                plot_seqprop_logo(pwm, iso, cut, annotate_peaks='max', sequence_template=self.sequence_template, figsize=figsize, width_ratios=[1, 8], logo_height=0.8, usage_unit='log', plot_start=plot_start, plot_end=plot_end)
